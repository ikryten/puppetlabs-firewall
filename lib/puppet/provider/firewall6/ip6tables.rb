Puppet::Type.type(:firewall6).provide :ip6tables, :parent => Puppet::Type.type(:firewall).provider(:iptables), :source => Puppet::Type.type(:firewall).provider(:iptables) do
  ...
  @known_booleans = [:ishasmorefrags, :islastfrag, :isfirstfrag, :rsource, :rdest, :reap, :rttl, :log_uid]

  # Create property methods dynamically
  (@resource_map.keys << :chain << :table << :action << :line).each do |property|
  ...
  @resource_list = [:table, :source, :destination, :iniface, :outiface,
    ...
    :jump, :todest, :tosource, :toports, :log_level, :log_prefix, :log_uid, :reject]

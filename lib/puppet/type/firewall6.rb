...
Puppet::Type.newtype(:firewall6) do
  ...
  feature :log_uid, "The ability to log userid of the program associated with the packet"
  feature :string_match, "The ability to match packets containing a certain string"
  ...
  newparam(:name) do
    ...
    # Keep rule names simple - they must start with a 5-digit number
    newvalues(/^\d{5}[[:alpha:][:digit:][:punct:][:space:]]+$/)
  end
  ...
  newparam(:log_uid, :boolean => true, :required_features => :log_uid) do
    desc <<-EOS
      When combined with jump => "LOG" logs the user id of the program
  ...
  newproperty(:string, :required_features => :string_match) do
    desc <<-EOS
      Match packets contaning a specific string
    EOS
    newvalues(/^\w+$/)
  end

  newproperty(:hex_string, :required_features => :string_match) do
    desc <<-EOS
      Match packets contaning a specific hex string
    EOS
    newvalues(/^\|\S+\|$/)
  end

  newproperty(:string_algo, :required_features => :string_match) do
    desc <<-EOS
      Algorithm to use for string matching ("bm" or "kmp")
    EOS
    newvalues(/^\w+$/)
  end

  newproperty(:string_from, :required_features => :string_match) do
    desc <<-EOS
      Starting offset when scanning the packet
    EOS
    newvalues(/^\d+$/)
    munge do |value|
      if resource[:string] && value.nil?
        "0"
      else
        value
      end
  ...
  newproperty(:string_to, :required_features => :string_match) do
    desc <<-EOS
      Ending offset when scanning the packet
    EOS
    newvalues(/^\d+$/)
  end

  newproperty(:string_offset, :required_features => :string_match) do
    desc <<-EOS
      Beginning and ending offset for string matching (TO:FROM)
    EOS
    newvalue(/^(\d+:\d+|:\d+|\d+:)$/)
    munge do |value|
      if value.match(/(\d+):/) then @resource[:string_from] = $1 end
      if value.match(/:(\d+)/) then @resource[:string_to] = $1 end
    end
  end

  newproperty(:ipset) do
    desc <<-EOS
      Match a host in the named ipset hash by src, dst, or both.
      Example values are: 'NCBI_PROD dst', 'NCBI_QA src'
    EOS
  end
...
#meslerd: This is causing a dependency cycle
